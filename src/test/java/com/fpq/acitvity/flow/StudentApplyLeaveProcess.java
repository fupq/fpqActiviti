 ...
 *
 */
class StudentApplyLeaveProcess {
	 ...
	 */
	@Test
	public void selectProcessState() {
		ProcessInstance processInstance = processEngine.getRuntimeService() //获取运行时server
		.createProcessInstanceQuery() //创建流程实例查询
		.processInstanceId("72501") //用流程实例id查询
		.singleResult(); //返回一条记录
		
		if(null != processInstance) {
			System.out.println("流程正在执行！流程名称："+processInstance.getProcessDefinitionName());
		}else {
	...
	public void findHistoryProcessInstanceId() {
		...
		.processInstanceId("50001") //根据任务id查询
		.finished() //查询已经完成的任务
		.list();
	...
	public void historyActInstanceList() {
		List<HistoricActivityInstance> historicActivityInstanceList = processEngine.getHistoryService() //获取历史的server
				...
				.processInstanceId("72501") //指定流程实例id
				.finished() //只查询完结的流程实例
				.list();
