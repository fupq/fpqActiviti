...
class AsignTaskMultiUser {
	...
	public void deployStudentLeaveApplyProcess() {
		...
		.addClasspathResource("diagrams/activiti_applyLeave_assignmentTask_multiUser.bpmn") //加载工作流资源文件
		.addClasspathResource("diagrams/activiti_applyLeave_assignmentTask_multiUser.png") //加载工作流资源文件
		.name("学生请假电子流_多用户任务分配_指定某人") //设置工作流程的名称
		.deploy();//部署
	...
	public void start(){
		ProcessInstance pi=processEngine.getRuntimeService() // 运行时Service
			.startProcessInstanceByKey("activitiAssignmentTaskMultiUser"); // 流程定义表的KEY字段值：myProcess
	...
	public void findTaskMultiUser(){
		List<Task> taskList=processEngine.getTaskService() // 任务相关Service
			.createTaskQuery() // 创建任务查询
			.taskCandidateUser("carson")//指定候选人
			.list();
		for(Task task:taskList){
			System.out.println("任务ID:"+task.getId()); 
			System.out.println("任务名称:"+task.getName());
