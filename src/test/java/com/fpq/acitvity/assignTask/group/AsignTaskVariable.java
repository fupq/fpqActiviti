 ...
 * 
 */
package com.fpq.acitvity.assignTask.group;
...
class AsignTaskVariable {
	...
	public void deployStudentLeaveApplyProcess() {
		...
		.addClasspathResource("diagrams/activiti_applyLeave_assignmentTask_group_Variable.bpmn") //加载工作流资源文件
		.addClasspathResource("diagrams/activiti_applyLeave_assignmentTask_group_Variable.png") //加载工作流资源文件
		.name("学生请假电子流_使用流程变量设置组（角色）分配任务") //设置工作流程的名称
		.deploy();//部署
	...
	public void start(){
		try {
			...
			map.put("groupId", "apply");
			ProcessInstance pi=processEngine.getRuntimeService() // 运行时Service
					.startProcessInstanceByKey("activitiAssignmentTaskGroupVariable", map); //代初始化变量方式启动流程
	...
	public void findTask(){
		List<Task> taskList=processEngine.getTaskService() // 任务相关Service
			...
			.taskCandidateUser("付柯翔") //根据组的成员id查询
			.list();
	...
	public void completeTaskSetVariable(){
		try {
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("groupId", "aprove");
			processEngine.getTaskService() // 任务相关Service
				.complete("390005", map); //完成任务时设置变量
		}catch(Exception e) {
			System.out.println(e.getMessage());
		}
	}
	...
	public void findHistoryTask() {
		...
		.processInstanceId("215001") //根据任务id查询
		.finished() //查询已经完成的任务
		.list();
